...
// Call OpenAI for summary template JSON
async function getOpenAISummaryJSON(text, referenceText, patientDetails) {
    const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    const prompt = `
You are a medical data extraction AI. Your job is to extract all possible blood test results from the following patient report text, match them to the reference ranges, and fill in as much detail as possible in the output JSON. If any field is missing in the report, use "Unknown" or "N/A". If a value is present but the unit or reference is missing, make a best guess based on the reference section. Do not skip any test you can find, even if the value or unit is unclear.

For the summary, categorize only the test panels (e.g., CBC, Liver Function, etc.) as 'normal', 'borderline', or 'abnormal' based on the overall panel's results:
- 'normal': all key parameters are well within healthy range.
- 'abnormal': one or more key parameters are clearly outside the healthy range.
- 'borderline': if any key parameter is close to the abnormal threshold, or trending toward abnormal, or is clinically critical but not yet fully abnormal, mark the panel as 'borderline'.

Do NOT leave the 'borderline' category empty if any test is near the borderline or trending toward abnormal. Use your medical knowledge to ensure at least one panel is marked 'borderline' if any are close to abnormal.
...
Return ONLY valid JSON in the following format (no explanation, no extra text):
{
  ...
  healthPanels: [
    {
      title: string, // panel name (from list above)
      status: string, // Normal/Borderline/Abnormal
      statusClass: string, // normal/borderline/abnormal
      iconClass: string, // e.g. fa-heart
      positionClass: string, // e.g. lipid-panel (leave blank, will be set in code)
      hasParameters: boolean,
      parameters: [
        { name: string, value: string, valueStatus: string } // valueStatus: normal, borderline, or abnormal
      ]
    }
    // ...repeat for each panel (always 9)
  ],
  normalTests: string, // comma-separated panel names only
  borderlineTests: string, // comma-separated panel names only
  abnormalTests: string, // comma-separated panel names only
...
IMPORTANT:
- Do NOT include any explanation or extra text, only the JSON object.
- Categorize only the panels (not individual parameters) as normal, borderline, or abnormal and fill the three summary fields with panel names only. Do not leave borderlineTests empty if any panel is near borderline.
`;
    const response = await openai.chat.completions.create({
        model: 'gpt-4.1',
        messages: [{ role: 'user', content: prompt }],
...
`;
    ...
    // --- SUMMARY PAGE LOGIC ---
    // Call OpenAI for summary JSON (panel-based, strict)
    let summaryData;
    try {
        let summaryJson = await getOpenAISummaryJSON(userText, referenceText, patientDetails);
        summaryJson = summaryJson.replace(/(\d{1,3}),(\d{3})/g, '$1$2'); // Clean commas
        const summaryMatch = summaryJson.match(/\{[\s\S]*\}/);
        if (summaryMatch) {
            const parsedSummary = JSON.parse(summaryMatch[0]);
            if (parsedSummary && parsedSummary.patientName) {
                if ('referenceNotes' in parsedSummary) delete parsedSummary.referenceNotes;
                summaryData = parsedSummary;
            } else {
                summaryData = {
                    patientName: gptJson.patient?.name || 'Unknown Patient',
                    accessionNumber: gptJson.report?.accessionNo || 'N/A',
                    gender: gptJson.patient?.gender || 'Unknown',
                    age: gptJson.patient?.age || 'N/A',
                    testDate: gptJson.report?.date || new Date().toLocaleDateString(),
                    healthPanels: gptJson.healthPanels || [],
                    normalTests: gptJson.normalTests || 'None',
                    borderlineTests: gptJson.borderlineTests || 'None',
                    abnormalTests: gptJson.abnormalTests || 'None'
                };
            }
        } else {
            summaryData = {
    ...
    // --- Use the same canonical panelOrder everywhere ---
    const panelOrder = [
        { key: 'cbc-panel', label: 'CBC', icon: 'fa-tint' },
        { key: 'liver-function-panel', label: 'Liver Function', icon: 'fa-procedures' },
        { key: 'renal-function-panel', label: 'Renal Function', icon: 'fa-kidneys' },
        { key: 'electrolytes-panel', label: 'Electrolytes', icon: 'fa-bolt' },
        { key: 'lipid-panel', label: 'Lipid Panel', icon: 'fa-oil-can' },
        { key: 'glucose-panel', label: 'Glucose', icon: 'fa-chart-line' },
        { key: 'thyroid-panel', label: 'Thyroid', icon: 'fa-shield-alt' },
        { key: 'calcium-panel', label: 'Calcium', icon: 'fa-pills' },
        { key: 'urinalysis-panel', label: 'Urinalysis', icon: 'fa-heartbeat' }
    ];
    if (Array.isArray(summaryData.healthPanels)) {
        summaryData.healthPanels = summaryData.healthPanels.map(panel => {
            const found = panelOrder.find(p => p.label.toLowerCase() === (panel.title || '').toLowerCase().trim());
            if (found) {
                panel.positionClass = found.key;
                panel.iconClass = 'fas ' + found.icon;
            } else {
                panel.positionClass = '';
                panel.iconClass = '';
            }
    ...
    // Use direct HTML rendering for Puppeteer
    const summaryHtml = await ejs.renderFile(path.join(__dirname, '../summary-template.ejs'), summaryData, { async: true });
    const testHtml = await ejs.renderFile(path.join(__dirname, '../test.ejs'), gptJson, { async: true });
    const combinedHtml = summaryHtml + '<div style="page-break-after: always;"></div>' + testHtml;
    try {
...
// Update PDF generation to use live server rendering
async function htmlToPdfLiveServer(url, outputPath) {
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
